...
CREATE OR REPLACE VIEW forum2._post_view AS
SELECT iddoc, content, create_date
FROM forum2._document JOIN forum2._post USING (iddoc);
CREATE OR REPLACE VIEW forum2._comment_view AS
SELECT iddoc, content, create_date
FROM forum2._document JOIN forum2._comment USING (iddoc);
CREATE OR REPLACE VIEW forum2._user_view AS
SELECT nickname, pass, email
FROM forum2._user;
